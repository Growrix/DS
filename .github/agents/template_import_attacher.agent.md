...
5. Wire the imported runtime's existing data, form, upload, navigation, and session-aware surfaces to those facades wherever the surface already exists in the imported UI.
...
7. Leave visual enhancement and non-contract feature expansion to post-import continuation.
...
2. Merge existing imported UI surfaces onto those facades without redesigning the public implementation.
3. Keep standalone fallback runnable when Foundation Core is offline.
4. Do not rewrite the imported visual structure during attach wiring.
...
4. Hand the template root to `Claude_Frontend_Agent` only for follow-up completion or enhancement work.
