      ...
      <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
        <div className='flex justify-between items-center h-20'>
          ...
          <nav className='hidden md:flex items-center gap-6'>
            {nav.map(l => <Link key={l.href} href={l.href} className='text-gray-600 hover:text-primary-500 font-medium transition-colors text-sm'>{l.label}</Link>)}
        ...
        {open && (
          <nav className='md:hidden py-4 border-t border-gray-100 flex flex-col gap-3'>
            {nav.map(l => <Link key={l.href} href={l.href} onClick={() => setOpen(false)} className='text-gray-700 hover:text-primary-500 font-medium py-1'>{l.label}</Link>)}
