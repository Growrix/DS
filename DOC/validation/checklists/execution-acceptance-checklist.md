...
- [ ] `frontend-execution-contract.json` provides execution-ready routes, nav surfaces, normalized tokens, content payloads, and release checks without requiring execution-time reverse engineering from prose.
...
- [ ] When execution reads frontend scope, generated code proves it consumed `frontend-execution-contract.json` as the deterministic handoff and preserved its route coverage.
...
- [ ] Project-specific DS implementation lives only in run-scoped clone under `DOC/output/runs/<timestamp>/codegen/<project-slug>/`.
- [ ] Dev/install/verify commands for DS-bound runs are executed from clone app root, not canonical DS root.
- [ ] If clone runtime failed, execution is classified blocked with explicit `CLONE_RUNTIME_UNAVAILABLE` evidence.
- [ ] DS clone contains only allowlisted runtime package contents (no canonical docs/history/output/git metadata).
