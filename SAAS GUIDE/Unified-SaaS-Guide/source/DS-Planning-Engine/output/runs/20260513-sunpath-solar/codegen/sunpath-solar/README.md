...
Before any extension or build, every AI agent (Copilot, Replit, Claude Code, custom orchestrators) MUST read:

1. **[`AGENTS.md`](./AGENTS.md)** — the agent entry point (this is the file Codex / OpenAI agent-SDK / Anthropic agents look for first).
2. **[`AI-AGENT-CONTRACT.md`](./AI-AGENT-CONTRACT.md)** — full rules (what's locked, what's extensible, how to add a section variant or industry preset, what triggers a build failure).
3. **[`.ai-scope.json`](./.ai-scope.json)** — machine-readable scope contract (locked / extensible / consumable / project-content glob lists).
4. **[`generated/ds.contract.json`](./generated/ds.contract.json)** — live registry: every section variant, archetype, motion preset, theme, and registered preset. This is the agent's primary retrieval index. Regenerate via `npm run ds:contract`.

Operating principle: this DS is a factory for client sites, not the deliverable itself. Per-client projects clone this repo, add a preset under `src/site/presets/`, and export the cloned app. The DS core is **locked** to keep that factory contract intact across projects.
