...
export default function Home() {
  ...
  return (
    <main className="mx-auto flex w-full max-w-7xl flex-1 flex-col gap-10 px-6 py-10 sm:px-8 lg:px-10">
      <section className="grid gap-6 rounded-[32px] border border-white/10 bg-[var(--surface)] p-8 shadow-[var(--shadow-panel)] lg:grid-cols-[1.2fr_0.8fr]">
        <div className="space-y-6">
          <span className="inline-flex rounded-full border border-[var(--border-strong)] px-4 py-1 text-sm font-medium text-[var(--accent)]">
        ...
        <div className="grid gap-4 rounded-[28px] border border-white/10 bg-[var(--panel)] p-6">
          <div>
      ...
      <section className="grid gap-6 lg:grid-cols-[1fr_1fr]">
        <article className="rounded-[28px] border border-white/10 bg-[var(--surface)] p-7 shadow-[var(--shadow-panel)]">
          ...
          <div className="mt-6 grid gap-4">
            {apiSurfaces.map((surface) => (
              <div key={surface.path} className="rounded-2xl border border-white/8 bg-[var(--panel)] p-4">
                <div className="flex flex-wrap items-center gap-3">
                  <span className="inline-flex min-w-16 justify-center rounded-full border border-[var(--border-strong)] px-2 py-1 text-xs font-semibold tracking-wide text-[var(--accent)]">
                    {surface.method}
                  </span>
                  <code className="text-sm text-[var(--foreground)]">{surface.path}</code>
                </div>
                <p className="mt-3 text-sm leading-6 text-[var(--muted)]">{surface.description}</p>
              </div>
            ))}
          </div>
        </article>
          ...
          <div className="mt-6 grid gap-4">
            {routes.map((route) => (
              <a
                key={route.href}
                href={route.href}
                className="rounded-2xl border border-white/8 bg-[var(--panel)] p-4 transition-transform hover:-translate-y-0.5"
              >
                <div className="text-base font-semibold">{route.label}</div>
                <div className="mt-2 text-sm text-[var(--muted)]">{route.href}</div>
              </a>
            ))}
          </div>
        </article>
...
function StatusRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between rounded-2xl border border-white/8 bg-[var(--surface-soft)] px-4 py-3">
